    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const [profileRes, addressRes, orderRes, productsRes] = await Promise.all([
        ...
        supabase
          .from("orders")
          .select(
            "id, status, total_amount, shipping_amount, customer_name, customer_email, shipping_address, created_at, order_items(id, product_id, product_name, quantity, price_at_purchase, line_item_meta)"
          )
          .eq("user_id", data.user.id)
          .order("created_at", { ascending: false }),
